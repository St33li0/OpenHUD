// Plugin.cpp
// https://raw.githubusercontent.com/thomas4f/mxbmrp2/refs/heads/master/mxbmrp2/Plugin.cpp
...
// Constructor
Plugin::Plugin()
	: configManager_(ConfigManager::getInstance())/*uc*//*,
...
void Plugin::onStartup(const std::string& savePath) {
	...
	/*uc*///keyPressHandler_ = std::make_unique<KeyPressHandler>([this]() { this->toggleDisplay(); }, HOTKEY);

	// timeTracker
	/*uc*///TimeTracker::getInstance().initialize(baseDir / DAT_FILE);

	// Initialize Discord Core
	useDiscordRichPresence_ = configManager_.getValue<bool>("enable_discord_rich_presence"); // Disabled/Unused
	if (useDiscordRichPresence_) {
		/*uc*///discordManager_.initialize(DISCORD_APP_ID);
	}
...
void Plugin::onShutdown() {
	...
	if (!bikeID_.empty() && !trackID_.empty()) {
		/*uc*///TimeTracker::getInstance().endRun(trackID_, bikeID_);
		/*uc*///TimeTracker::getInstance().save();
	}

	// Discord
	if (useDiscordRichPresence_) {
		/*uc*///discordManager_.finalize();
	}

	// HTML Export
	if (useHtmlExport_) {
		/*uc*///HtmlWriter::atomicWrite(htmlPath_, HtmlWriter::renderNoData());
	}
}
	...
	while (runPeriodicTask_) {
		{
			...
			if (connectionType_ == "Client") {
				/*uc*///serverPing_ = MemReaderHelpers::getRemoteServerPing();
			}
			if (connectionType_ == "Host" || connectionType_ == "Client") {
				/*uc*///serverClients_ = MemReaderHelpers::getServerClientsCount();
			...
			/*uc*/// if (playerActivity_ == "On Track" && !isPaused_) {
			/*uc*/// 	updateDataKeys({
			/*uc*/// 		{ "combo_time", TimeTracker::getInstance().getComboTime() },
			/*uc*/// 		{ "total_time", TimeTracker::getInstance().getTotalTime() },
			/*uc*/// 		{"remaining_tearoffs", MemReaderHelpers::getRemainingTearoffs(connectionType_)}
			/*uc*/// 	});
			/*uc*/// }

			updateDataKeys({
				/*uc*///{"discord_status", discordManager_.getConnectionStateString()}
			});

			// Export JSON
			/*uc*///if (useJsonExport_) {
			/*uc*///	std::string js = JsonWriter::renderJson(
			/*uc*///		allDataKeys_,
			/*uc*///		configKeyToDisplayNameMap,
			/*uc*///		configManager_);
			/*uc*///	if (js != lastJson_) {
			/*uc*///		try {
			/*uc*///			JsonWriter::atomicWrite(jsonPath_, js);
			/*uc*///			lastJson_ = std::move(js);
			/*uc*///		}
			/*uc*///		catch (const std::exception& e) {
			/*uc*///			Logger::getInstance().log(
			/*uc*///				std::string("JSON write failed: ") + e.what());
			/*uc*///		}
			/*uc*///	}
			/*uc*///}

			// Export HTML
			/*uc*///if (useHtmlExport_) {
			/*uc*///	std::string html = HtmlWriter::renderHtml(
			/*uc*///		allDataKeys_,
			/*uc*///		configKeyToDisplayNameMap,
			/*uc*///		configManager_);
			/*uc*///	if (html != lastHtml_) {
			/*uc*///		try {
			/*uc*///			HtmlWriter::atomicWrite(htmlPath_, html);
			/*uc*///			lastHtml_ = std::move(html);
			/*uc*///		}
			/*uc*///		catch (const std::exception& e) {
			/*uc*///			Logger::getInstance().log(
			/*uc*///				std::string("HTML write failed: ") + e.what());
			/*uc*///		}
			/*uc*///	}
			/*uc*///}
		}
		...
		if (useDiscordRichPresence_) {
			...
			/*uc*///discordManager_.tick(details, state, partySize, partyMax);
		}
...
void Plugin::onRunInit(const SPluginsBikeSession_t& sessionData) {
	...
	/*uc*///TimeTracker::getInstance().startRun(trackID_, bikeID_, bikeCategory_, setupName);
	...
	updateDataKeys({
		...
		/*uc*///{"combo_time", TimeTracker::getInstance().getComboTime()},
		/*uc*///{"total_time", TimeTracker::getInstance().getTotalTime()},
		/*uc*///{"session_pb", TimeTracker::getInstance().getSessionPB()},
		/*uc*///{"alltime_pb", TimeTracker::getInstance().getAlltimePB()},
        /*uc*///{"combo_laps", TimeTracker::getInstance().getComboLapCount()},
        /*uc*///{"total_laps", TimeTracker::getInstance().getTotalLapCount()},
        {"cut_penalty", "0s"}
	});
}
...
void Plugin::onRaceEvent(const SPluginsRaceEvent_t& raceEvent) {
	...
	/*uc*///std::string localServerName = MemReaderHelpers::getLocalServerName();

	/*uc*///if (!localServerName.empty()) { // Definitely a host
	/*uc*///	connectionType_ = "Host";
	/*uc*///	serverName_ = localServerName;
	/*uc*///	serverPassword_ = MemReaderHelpers::getLocalServerPassword();
	/*uc*///	serverLocation_ = MemReaderHelpers::getLocalServerLocation();
	/*uc*///	serverClientsMax_ = MemReaderHelpers::getLocalServerClientsMax();
	/*uc*///}
	/*uc*///else { // Possibly a client
	/*uc*///	auto remoteServerSocketAddress_ = MemReaderHelpers::getRemoteServerSocketAddress();
	/*uc*///	remoteServerIPv6Address_ = PluginHelpers::formatIPv6MappedIPv4(remoteServerSocketAddress_);
	/*uc*///	if (!remoteServerSocketAddress_.empty()) { // Definitely a client
	/*uc*///		connectionType_ = "Client";
	/*uc*///		serverPassword_ = MemReaderHelpers::getRemoteServerPassword();
	/*uc*///		std::string connectURIServerName = PluginHelpers::getServerNameFromConnectURI(MemReaderHelpers::getConnectURIString());
	/*uc*///		if (!connectURIServerName.empty()) { // Client is using connect URI
	/*uc*///			serverName_ = connectURIServerName;
	/*uc*///			// Can't get data from server list when using connect URI :(
	/*uc*///			serverLocation_ = "Unknown";
	/*uc*///			serverClientsMax_ = 0;
	/*uc*///		}
	/*uc*///		else { // Client connected via server browser
	/*uc*///			std::tie(remoteServerIPv6AddressMemoryAddress_, serverName_) =
	/*uc*///				MemReaderHelpers::getRemoteServerNameAndAddress(remoteServerSocketAddress_);
	/*uc*///			if (serverName_.empty()) serverName_ = "Unknown";
	/*uc*///			serverLocation_ = MemReaderHelpers::getRemoteServerLocation(remoteServerIPv6AddressMemoryAddress_);
	/*uc*///			serverClientsMax_ = MemReaderHelpers::getRemoteServerClientsMax(remoteServerIPv6AddressMemoryAddress_);
	/*uc*///			std::string connectURIString = PluginHelpers::buildConnectURIString(
	/*uc*///				remoteServerIPv6Address_,
	/*uc*///				serverName_,
	/*uc*///				serverPassword_,
	/*uc*///				MemReaderHelpers::getServerTrackID(),
	/*uc*///				MemReaderHelpers::getServerCategories()
	/*uc*///			);
	/*uc*///			Logger::getInstance().log("connectURIString: " + connectURIString);
	/*uc*///		}
	/*uc*///	}
	/*uc*///	else { // Offline
	/*uc*///		connectionType_ = "Offline";
	/*uc*///	}
	/*uc*///}
	...
	updateDataKeys({
		...
		/*uc*///{"track_deformation", MemReaderHelpers::getTrackDeformation() + "x"}
	});
}
...
void Plugin::onEventDeinit() {
	...
	/*uc*///TimeTracker::getInstance().resetSessionPB();
	/*uc*///TimeTracker::getInstance().save();
...
void Plugin::onRunLap(const SPluginsBikeLap_t& lapData) {
	...
	if (!lapData.m_iInvalid && lapData.m_iLapTime > 0) {
		...
		/*uc*///TimeTracker::getInstance().recordLap(trackID_, bikeID_, lapData.m_iLapTime, currentLapSplitsMs_);
		/*uc*///TimeTracker::getInstance().save();
...
void Plugin::toggleDisplay() {
	...
	if (newUseJsonExport && !useJsonExport_) {
		...
		/*uc*///JsonWriter::atomicWrite(jsonPath_, JsonWriter::renderNoData());
