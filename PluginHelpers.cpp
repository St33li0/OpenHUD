// PluginHelpers.cpp
// https://raw.githubusercontent.com/thomas4f/mxbmrp2/refs/heads/master/mxbmrp2/PluginHelpers.cpp
...
namespace PluginHelpers {
    ...
    /*uc*///    // Need at least: 2�0xFF + 4 IPv4 bytes + 2 port bytes
    /*uc*///    if (ipv6.size() < 8) {
    /*uc*///        return "Invalid";
    /*uc*///    }

    /*uc*///    uint16_t hext1, hext2, port;
    /*uc*///    // copy the raw bytes (network order) into our words
    /*uc*///    std::memcpy(&hext1, ipv6.data() + 2, sizeof(hext1));  // first half of IPv4
    /*uc*///    std::memcpy(&hext2, ipv6.data() + 4, sizeof(hext2));  // second half of IPv4
    /*uc*///    std::memcpy(&port, ipv6.data() + 6, sizeof(port));   // port

    /*uc*///    // convert from network byte order to host
    /*uc*///    hext1 = ntohs(hext1);
    /*uc*///    hext2 = ntohs(hext2);
    /*uc*///    port = ntohs(port);

    /*uc*///    std::ostringstream oss;
    /*uc*///    oss << "[::ffff:"
    /*uc*///        << std::hex << hext1 << ":" << hext2
    /*uc*///        << std::dec << "]:" << port
    /*uc*///        << "/[::1]:" << port;
    /*uc*///    return oss.str();
    /*uc*///}
